...
- **Geometric Jacobian:** \(J_g = [J_v; J_w]\) based on joint axes \(z_i\) and positions \(o_i\)  
- **Analytic Jacobian:** \(J_a = \mathrm{diag}(I, B^{-1}) J_g\), where \(B\) is the Z–Y–X Euler transformation matrix.
...
This is an extended version of **2-DOF planar manipulator** control, implementing task-space PD control based on center of mass (CoM) regulation.
